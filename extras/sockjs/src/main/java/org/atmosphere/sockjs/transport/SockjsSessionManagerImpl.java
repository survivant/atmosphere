...
import org.atmosphere.sockjs.HeartBeatSessionMonitor;
...
import org.atmosphere.sockjs.TimeoutSessionMonitor;
 ...
 */
public class SockjsSessionManagerImpl implements SockjsSessionManager, SockjsSessionFactory {

    private static final Logger logger = LoggerFactory.getLogger(SockjsSessionManagerImpl.class);

    private static final int SESSION_ID_LENGTH = 20;
    private static Random random = new SecureRandom();
    private ConcurrentMap<String, SockjsSession> sockjsSessions = new ConcurrentHashMap<String, SockjsSession>();
    ...
    public static final String HEARTBEAT_DATA = "h";
    ...
    @Override
    public SockjsSession createSession(AtmosphereResourceImpl resource, AtmosphereHandler inbound, String sessionid) {
        SessionImpl impl = new SessionImpl(sessionid, resource, inbound, getTimeout(), getHeartbeatInterval(), getRequestSuspendTime());
        sockjsSessions.put(impl.getSessionId(), impl);
        return impl;
    }

    @Override
    public SockjsSession getSession(String sessionId) {
        return sockjsSessions.get(sessionId);
    }
    ...
    private class SessionImpl implements SockjsSession {
        private final String sessionId;

        private AtmosphereResourceImpl resource = null;
        private AtmosphereHandler atmosphereHandler;
        private SockjsSessionOutbound handler = null;
        ...
        SessionImpl(String sessionId, AtmosphereResourceImpl resource, AtmosphereHandler atmosphereHandler, long timeout, long heartBeatInterval, long requestSuspendTime) {
            ...
            heartBeatSessionMonitor.setDelay(1000);
            timeoutSessionMonitor.setDelay(timeout);
        }

        @Override
        public String generateRandomString(int length) {
            return SockjsSessionManagerImpl.generateRandomString(length);
        }
        ...
        @Override
        public SockjsSessionOutbound getTransportHandler() {
            return handler;
        }
        ...
        public void sendHeartBeat() {
            ...
            try {
                handler.sendMessage(HEARTBEAT_DATA);
            } catch (Exception e) {
        ...
        @Override
        public void onConnect(AtmosphereResourceImpl resource, SockjsSessionOutbound handler) {
            if (handler == null) {
                state = ConnectionState.CLOSED;
                atmosphereHandler = null;
                sockjsSessions.remove(sessionId);
                ...
                try {
                    ...
                    // for the Broadcaster
                    resource.getRequest().setAttribute(SockjsAtmosphereHandler.SOCKJS_SESSION_ID, sessionId);
                    resource.getRequest().setAttribute(SockjsAtmosphereHandler.SOCKJS_SESSION_OUTBOUND, handler);

                    startHeartbeatTimer();
                    synchronized (atmosphereHandler) {
                        if (SockjsAtmosphereHandler.class.isAssignableFrom(atmosphereHandler.getClass())) {
                        	SockjsAtmosphereHandler.class.cast(atmosphereHandler).onConnect(resource, handler);
                        } else {
        ...
        @Override
        public void onMessage(AtmosphereResourceImpl resource, SockjsSessionOutbound outbound, String message) {
            startHeartbeatTimer();
            
            System.out.println("onMessage SocjksSessionManager=" + message);

            if (atmosphereHandler != null && message != null) {
                try {
                    synchronized (atmosphereHandler) {
                        if (SockjsAtmosphereHandler.class.isAssignableFrom(atmosphereHandler.getClass())) {
                        	SockjsAtmosphereHandler.class.cast(atmosphereHandler).onMessage(resource, outbound, message);
                        } else {
                            SockjsProtocol p = mapper.readValue(message, SockjsProtocol.class);

                            for (String msg : p.getArgs()) {
                                AtmosphereRequest r = resource.getRequest();
                                r.setAttribute(SockjsProtocol.class.getName(), p);
        ...
        public void onDisconnect(DisconnectReason reason) {
            ...
            if (atmosphereHandler != null) {
                ...
                try {
                    synchronized (atmosphereHandler) {
                        if (SockjsAtmosphereHandler.class.isAssignableFrom(atmosphereHandler.getClass())) {
                        	SockjsAtmosphereHandler.class.cast(atmosphereHandler).onDisconnect(resource, handler, reason);
                        } else {
    ...
    public static final class SockjsProtocol {
        public String name;
        ...
        public SockjsProtocol addArgs(String s) {
            if (args == null) {
        ...
        public SockjsProtocol clearArgs(){
            args.clear();
            return this;
        }
        ...
        public String toString() {
            return "SockjsProtocol [name=" + name + ", args=" + args + "]";
        }
    }
}
