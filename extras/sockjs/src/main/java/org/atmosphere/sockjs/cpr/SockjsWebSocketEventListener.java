 ...
 */
public class SockjsWebSocketEventListener extends WebSocketEventListenerAdapter {

    private static final Logger logger = LoggerFactory.getLogger(SockjsWebSocketEventListener.class);
    private SockjsWebSocketSessionWrapper sessionWrapper = null;

    public void setSessionWrapper(SockjsWebSocketSessionWrapper sessionWrapper) {
    ...
    public void onMessage(WebSocketEvent event) {
        ...
        if (!sessionWrapper.isInitiated()) {
            ...
            List<SockjsPacketImpl> messages = null;
            try {
                messages = SockjsPacketImpl.parse(event.message());
            } catch (SockjsException e) {
                logger.warn("", e);
            }

            if (messages != null && !messages.isEmpty()) {
                SockjsSession session = sessionWrapper.getSession();
    ...
    public void onConnect(WebSocketEvent event) {
        ...
        try {
            event.webSocket().write(new SockjsPacketImpl("-4").toString());
        } catch (IOException e) {
            e.printStackTrace();
            sessionWrapper.getSession().onShutdown();
        }
